...
<section class="max-w-6xl mx-auto px-4 py-16">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <a href="{{ route('shop', ['category' => 'shoes']) }}" class="relative overflow-hidden rounded-lg group">
        ...
        <a href="{{ route('shop', ['category' => 'perfumes']) }}" class="relative overflow-hidden rounded-lg group">
            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition">
               <img src="{{ asset('storage/' . ($product->coverImage->image_path ?? $product->images->first()->image_path ?? 'placeholder.jpg')) }}"
                ...
                <div class="p-4">
                    ...
                    <p class="text-sm text-gray-600">USD {{ number_format($product->sale_price ?? $product->price, 2) }}</p>
                </div>
            </div>
        @endforeach
    </div>
            ...
            @foreach ($testimonials as $feedback)
                <div class="min-w-[300px] bg-white rounded-lg p-4 shadow-sm">
                    <div class="flex items-center gap-3 mb-3">
                        <img src="{{ $feedback->avatar }}" class="w-10 h-10 rounded-full">
                        <div>
